...
isJawClenched = True
isFacingGizmo = False
notFacingGizmoforTenSeconds = False
...
async def await_ten_seconds():
    # i represents number of seconds not looking at gizmo
    i = 0
    # loop forever
    while True:
        # if not facing gizmo, increment i
        if not isFacingGizmo:
            i += 1
            # if not looking at gizmo for 10 or more seconds, set that boolean to true
            if i >= 10:
                notFacingGizmoforTenSeconds = True
        # else, they are looking at gizmo, set the boolean to false and reset i
        else:
            notFacingGizmoforTenSeconds = False
            i = 0
        # stop this coroutine for a second. this causes the loop to run about once a second
...
async def determineCommand(isJawClenched, isFacingGizmo, gizmoClient):
    ...
    elif not isFacingGizmo and notFacingGizmoforTenSeconds:
        # move backward
...
async def main():
    ...
    async with asyncio.TaskGroup() as tg:
        ...
        wait_ten_seconds = tg.create_task(await_ten_seconds())
